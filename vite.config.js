...
// Backend en desarrollo para Atria Edificios
      ...
      if (req.url === '/api/kommo' && req.method === 'POST') {
        ...
        req.on('end', async () => {
          try {
            ...
            // --- 1. Preparar el Lead con tag "edificios" ---
            const leadName = `Edificios - ${data.profile || 'Inversor'}: ${data.name}`;
                  ...
                  "contacts": [
                    {
                      ...
                      "custom_fields_values": [
                        ...
                        ...(data.phone ? [{ "field_code": "PHONE", "values": [{ "value": data.phone }] }] : [])
                      ]
                    }
                  ]
                }
            ...
            if (kommoResponse.ok) {
              ...
              try {
                ...
                console.log(`[ATRIA EDIFICIOS] Lead created: ${leadId} in pipeline 13404455 with tag "edificios"`);

                const noteText = `📌 LEAD ATRIA EDIFICIOS\n\nNombre: ${data.name}\nEmail: ${data.email}\nTeléfono: ${data.phone || 'No proporcionado'}\nPerfil inversor: ${data.profile || 'No especificado'}\nTipo de operación: ${data.operationType || 'No especificado'}\nVolumen orientativo: ${data.volume || 'No especificado'}\n\nFuente: Landing Page Atria Edificios`;
                ...
                console.log(`[ATRIA EDIFICIOS] Note attached: ${noteResp.status}`);
              } catch (noteErr) {
                console.error("[ATRIA EDIFICIOS] Note attachment failed:", noteErr);
              }
